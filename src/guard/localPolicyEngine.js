...
export class LocalPolicyEngine {
  ...
  async checkCrossSourceConflict(action, ctx) {
    const r = this.rules.cross_source_conflict
    if (!r?.appliesTo?.includes(action.type)) return null
    if (!action.record || !ctx.existingLoanMap) return null
    
    const existing = await ctx.existingLoanMap.get(action.record.loan_id)
  ...
  async evaluate(action, ctx = {}) {
    ...
    const conflictCheck = await this.checkCrossSourceConflict(action, context)
