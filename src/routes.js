    ...
    const failedRows = []
      ...
      try {
        if (sourceType === 'document_manifest') {
          ...
          // Memory-efficient: only load loan_ids (not full rows) for duplicate detection
          const existingIdRows = await db.all(`SELECT loan_id FROM loans`)
          const existingLoanIds = new Set(existingIdRows.map(r => r.loan_id))
          // Free the id rows array immediately
          existingIdRows.length = 0

          // Lazy per-record lookup for cross-source conflict (avoids loading all rows)
          const _loanCache = new Map()
          const existingLoanMap = {
            get: async (loanId) => {
              if (_loanCache.has(loanId)) return _loanCache.get(loanId)
              const row = await db.get(`SELECT loan_id, source_system, current_balance, payment_status FROM loans WHERE loan_id = ? LIMIT 1`, [loanId])
              _loanCache.set(loanId, row || null)
              return row || null
            }
          }
