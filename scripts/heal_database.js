...
async function main() {
  ...
  console.log('1. Pruning empty dummy stress batches (stress_1000 & stress_5000)...')
  const dummyBatches = await db.all(`SELECT id FROM upload_batches WHERE filename IN ('stress_1000.csv', 'stress_5000.csv')`)
  for (const b of dummyBatches) {
    await db.run(`DELETE FROM loans WHERE upload_batch_id = ?`, [b.id])
    await db.run(`DELETE FROM upload_batches WHERE id = ?`, [b.id])
  }

  console.log('2. Seeding 16 realistic open exceptions for Exception Reviewer...')
  ...
  // Clear existing exceptions to avoid stale state
  await db.run(`DELETE FROM exceptions`)
  ...
  console.log(`  -> Inserted ${exceptionDefinitions.length} exceptions.`)

  console.log('3. Seeding verified loans for Data Consumer...')
  const cleanLoans = await db.all(`SELECT * FROM loans WHERE validation_status = 'valid' LIMIT 55`)
  console.log(`  -> Found ${cleanLoans.length} clean loans to verify.`)
  ...
  console.log('4. Rebuilding contiguous cryptographic audit ledger (audit_logs)...')
  ...
  console.log(`  -> Audit ledger reconstructed with ${seq - 1} hash-chained entries.`)

  console.log('5. Optimizing and VACUUMing database...')
  await db.exec(`
  ...
  // Verification checks
  const exCount = (await db.get(`SELECT count(*) as c FROM exceptions WHERE status = 'open'`)).c
  const verCount = (await db.get(`SELECT count(*) as c FROM loans WHERE validation_status = 'verified' AND is_verified = 1`)).c
  const valCount = (await db.get(`SELECT count(*) as c FROM loans WHERE validation_status = 'valid'`)).c
  const totLoans = (await db.get(`SELECT count(*) as c FROM loans WHERE validation_status != 'pending'`)).c
  const auditCount = (await db.get(`SELECT count(*) as c FROM audit_logs`)).c

  console.log('\n--- FINAL HEALED DATABASE METRICS ---')
  console.log(`Total Active Loans: ${totLoans}`)
  console.log(`Open Exceptions:    ${exCount}`)
  console.log(`Verified Loans:     ${verCount}`)
  console.log(`Valid Clean Loans:  ${valCount}`)
  console.log(`Audit Ledger Logs:  ${auditCount}`)
  console.log(`Quality Score:      ${Math.round(((verCount + valCount) / totLoans) * 100)}%`)
  console.log('-------------------------------------\n')
