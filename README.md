...
Follow these simple steps to experience the complete end-to-end workflow:

1. **Ingest a Messy Tape (Operator Persona):**
   - On the Data Operator page, click the **Adversarial Tape** or **Large 3k Tape** button in the Quick Test Datasets card.
   - Click **Start Validation Pipeline**. Watch the system process thousands of records and isolate the anomalies.

2. **Diagnose & Fix with AI (Reviewer Persona):**
   - Switch to **Exception Reviewer** in the sidebar.
   - Click on the first flagged exception in the queue.
   - Read the **AI Copilot Diagnostic Analysis**. Notice the side-by-side diff comparing the corrupt value with the AI's suggested fix.
   - Click **Apply Suggestion**, click the **+ Accepted AI** note preset, and click **Approve & Verify**.

3. **Inspect the Immutable Audit Trail (Consumer Persona):**
   - Switch to **Data Consumer** in the sidebar.
   - Your newly resolved loan now appears in the verified portfolio.
   - Click **Inspect Chain** on the loan row to open the Cryptographic Proof drawer.
   - Click **Verify SHA-256** to see the live mathematical verification check pass.
   - Click **Governed Export (CSV)** or **JSON** to download the clean data.

4. **Review Agentic Coding Documentation:**
   - Click **AI Dev Log** in the sidebar to review the mandatory development log.
